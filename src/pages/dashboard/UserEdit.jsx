...
const UserEdit = () => {
    ...
    const handleDocumentImgOnChange = async (ev) => {
        if (ev.type.includes("image")) {
            const formData = new FormData();
            formData.append('file', ev)
            const imageData = await axiosPrivate.post(INDIVIDUAL_API_URL.documentUpload, formData)
            setDocImage(imageData?.data?.file)
        }
    };
                ...
                <div className="container-fluid">
                  ...
                  {/* <div className="row">
                        <div className='col'>
                            ...
                            {userData?.isMark === 1 ? <AppTooltip title="is mark on" placement="bottom">
                                <Button
                                    className="ms-4 app_bg_primary text-lowercase"
                                    variant="contained"
                                    component="label"
                                    onClick={(e) => updateIdMarkRequest(userData?._id)}
                                > is mark </Button>
                            </AppTooltip> : ""}
                        </div>
                    </div>  */}
                        ...
                        <div className='d-flex col-md-4 flex-column mt-4'>
                            ...
                            <div className='mb-4'>
                                <Field
                                    ...
                                    component={PhoneInputs}
                                    disabled={true}
                                    data={userData?.referenceUser?.[1] ? userData?.referenceUser?.[1]?.mobile.text : ""}
                                />
                            </div>
                            <div className='mb-4'>
                                <TextField
